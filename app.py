...
if st.session_state.page == "reports":
    ...
    """.format(st.session_state.full_name), unsafe_allow_html=True)
    ...
    total_cases = len(filtered[filtered["status"] == "متداولة"])

    total_judgments = len(
        filtered[filtered["judgment_result"].isin(["لصالح الهيئة", "ضد الهيئة"])]
    )

    win_cases = len(filtered[filtered["judgment_result"] == "لصالح الهيئة"])

    lose_cases = len(filtered[filtered["judgment_result"] == "ضد الهيئة"])

    st.markdown("## 📊 الإحصائيات")

    col1, col2, col3, col4 = st.columns(4)

    col1.metric("إجمالي القضايا المتداولة", total_cases)
    col2.metric("إجمالي الأحكام الصادرة", total_judgments)
    col3.metric("الأحكام لصالح الهيئة", win_cases)
    col4.metric("الأحكام ضد الهيئة", lose_cases)
    ...
    data1 = filtered[filtered["status"] == "متداولة"]
    ...
    st.markdown("## ✅ الأحكام لصالح الهيئة")

    data3 = filtered[filtered["judgment_result"] == "لصالح الهيئة"]

    if len(data3) == 0:
        st.info("لا توجد أحكام لصالح الهيئة")
    else:
    ...
    st.markdown("## ❌ الأحكام ضد الهيئة")

    data4 = filtered[filtered["judgment_result"] == "ضد الهيئة"]

    if len(data4) == 0:
        st.info("لا توجد أحكام ضد الهيئة")
    else:
