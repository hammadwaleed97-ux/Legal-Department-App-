...
with col2:
    ...
    if st.button("⚖️ تسجيل القضايا", key="btn1"):
        st.session_state.page = "cases"

    if st.button("🔔 التنبيهات", key="btn2"):
        st.session_state.page = "alerts"

    if st.button("📊 التقارير", key="btn3"):
        st.session_state.page = "reports"

    if st.button("📂 أرشيف القضايا", key="btn4"):
        st.session_state.page = "archive"

    if st.button("🔍 البحث عن دعوى", key="btn5"):
        st.session_state.page = "search"

    if st.button("❌ القضايا المحذوفة", key="btn6"):
...
if st.session_state.page == "cases":
    ...
    litigation_type = st.selectbox(
        "نوع الإجراء",
        ["دعوى", "استئناف", "نقض"], key="s1"
    )

    claimant_type = st.selectbox(
        "صفة الخصم الأول",
        ["المدعى", "المستأنف", "الطاعن"], key="s2"
    )

    claimant = st.text_input(
        "اسم الخصم الأول", key="t1"
    )

    defendant_type = st.selectbox(
        "صفة الخصم الثاني",
        ["المدعى عليه", "المستأنف ضده", "المطعون ضده"], key="s3"
    )

    defendant = st.text_input(
        "اسم الخصم الثاني", key="t2"
    )
    ...
    court = st.selectbox(
        "المحكمة",
        ["الابتدائية", "الاستئناف", "النقض", "إدارية", "قضاء إدارى", "إدارية عليا"], key="s4"
    )
    ...
    judgment_result = st.selectbox(
        "نتيجة الدعوى",
        ["متداولة", "لصالح الهيئة", "ضد الهيئة"], key="s5"
    )

    mobile = st.text_input(
        "رقم الموبايل لإرسال التنبيهات", key="t11"
    )
