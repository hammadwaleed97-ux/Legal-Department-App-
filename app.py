    ...
    if cases_df.empty:
        ...
        for _, row in cases_df.iterrows():
            ...
            with st.expander(title):
                ...
                if case_status == "ضد الهيئة":

                    judgment_date = st.date_input(
                        "تاريخ الحكم",
                        key=f"judgment_date_{row['id']}"
                    )
                ...
                if history_df.empty:
                    ...
                    for _, h in history_df.iterrows():
                        ...
                        st.write(
                            f"الحالة : {h['case_status']}"
                        )

                        if h["status_reason"]:

                            st.write(
                                f"السبب : {h['status_reason']}"
                            )

                        if h["judgment_date"]:

                            st.write(
                                f"تاريخ الحكم : {h['judgment_date']}"
                            )
