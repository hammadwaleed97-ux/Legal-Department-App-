...
CREATE TABLE IF NOT EXISTS cases(
    ...
    whatsapp_enabled INTEGER DEFAULT 0, whatsapp_number TEXT, created_at TEXT
);
...
if st.session_state.page == "home":
    ...
    with c:
        if st.button("⚖️ تسجيل قضية جديدة", use_container_width=True):
...
if st.session_state.page == "cases":
    ...
    # يمكنك إضافة النموذج الكامل هنا من النسخ السابقة
    st.info("صفحة التسجيل - أضف النموذج الكامل إذا أردت")
    if st.button("⬅ العودة"):
...
if st.session_state.page == "inventory":
    ...
    cur.execute("""
        SELECT c.id, c.case_number, c.judicial_year, c.circuit, c.case_category,
               ...
               (SELECT session_date FROM sessions s WHERE s.case_id = c.id ORDER BY s.session_date DESC LIMIT 1),
               (SELECT procedure FROM sessions s WHERE s.case_id = c.id ORDER BY s.session_date DESC LIMIT 1)
    ...
    if not rows:
        ...
        for row in rows:
            case_id = row[0]
            cols = st.columns([1.8, 1, 1.2, 1.5, 2, 1.3, 3.5, 1.8, 2.5, 1,1])
            
            cols[0].write(row[1])  # رقم الدعوى
            cols[1].write(row[2])  # السنة
            cols[2].write(row[3])  # الدائرة
            cols[3].write(row[4])  # النوع
            cols[4].write(row[5])  # المحكمة
            cols[5].write(row[6] or "-")  # المأمورية
            cols[6].write(f"{row[7]} ضد {row[8]}")  # الخصوم
            cols[7].write(row[9] or "-")   # آخر جلسة
            cols[8].write((row[10] or "-")[:50])  # سبب الجلسة

            with cols[9]:
                if st.button("📂 فتح", key=f"open{case_id}"):
                    st.session_state.selected_case = case_id
                    st.session_state.page = "case_details"
                    st.rerun()
            with cols[10]:
                if st.button("🗑", key=f"del{case_id}"):
...
if st.session_state.page in ["case_details", "edit_case", "reports"]:
    st.info("هذه الصفحة تحت التطوير")
