...
if st.session_state.page == "cases":
    ...
    st.info(
        f"المستخدم الحالي: {st.session_state.full_name}"
    )
...
st.markdown("---")  # 🔴 فاصل عن الجزء اللي قبله
    ...
    to_date = st.date_input("حتى تاريخ", value=date.today())
    ...
    total_judged = len(filtered[filtered["judgment_result"].isin(["لصالح الهيئة", "ضد الهيئة"])])
    win = len(filtered[filtered["judgment_result"] == "لصالح الهيئة"])
    lose = len(filtered[filtered["judgment_result"] == "ضد الهيئة"])

    st.markdown("## 📊 الإحصائيات")

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("إجمالي القضايا", total_cases)
    c2.metric("الأحكام الصادرة", total_judged)
    c3.metric("لصالح الهيئة", win)
    c4.metric("ضد الهيئة", lose)
    ...
    # بيان الدعاوى
    # =========================
    st.markdown("## 📋 بيان الدعاوى المتداولة")

    cases = filtered[
        filtered["status"].isin(["متداولة", "محجوزة للحكم"])
    ]
    ...
    # بيان الأحكام
    # =========================
    st.markdown("## ⚖️ بيان الأحكام الصادرة")

    judgments = filtered[
        filtered["judgment_result"].isin(["لصالح الهيئة", "ضد الهيئة"])
    ]
    ...
    def make_word():
        ...
        doc.add_heading("التقرير القضائي", 0)
        ...
        doc.add_paragraph(f"المستخدم: {st.session_state.full_name}")

        doc.add_heading("بيان الدعاوى", level=1)

        for _, r in cases.iterrows():
            doc.add_paragraph(
                f"{r['case_no']}/{r['judicial_year']} - {r['claimant']} ضد {r['defendant']} - {r['subject']}"
            )

        doc.add_heading("بيان الأحكام", level=1)

        for _, r in judgments.iterrows():
            doc.add_paragraph(
                f"{r['case_no']}/{r['judicial_year']} - {r['judgment_result']}"
            )
    ...
    def make_pdf():
        ...
        p.drawString(180, 800, "التقرير القضائي")

        y = 770
        p.setFont("Helvetica", 10)

        p.drawString(50, y, f"المستخدم: {st.session_state.full_name}")
        y -= 20

        p.drawString(50, y, f"إجمالي القضايا: {total_cases}")
        y -= 30

        p.drawString(50, y, "بيان الدعاوى:")
        y -= 20
        ...
        for _, r in cases.iterrows():
            p.drawString(
                50,
                y,
                f"{r['case_no']}/{r['judicial_year']} - {r['claimant']} ضد {r['defendant']}"
            )
            y -= 15
            if y < 50:
                p.showPage()
