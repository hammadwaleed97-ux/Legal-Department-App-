...
# تهيئة البيانات - إضافة حقل التاريخ كقيمة افتراضية لضمان عدم وجود فراغ
...
with tab1:
    with st.form("case_form", clear_on_submit=True):
        ...
        with col1:
            p1_type = st.selectbox("صفة الطرف الأول", ["مدعى", "مدعى عليه", "مستأنف", "مستأنف ضده", "طاعن", "مطعون ضده"], key="p1t")
            p1_name = st.text_input("اسم الطرف الأول")
            p2_type = st.selectbox("صفة الطرف الثاني", ["مدعى", "مدعى عليه", "مستأنف", "مستأنف ضده", "طاعن", "مطعون ضده"], key="p2t")
        ...
        if st.form_submit_button("حفظ القضية"):
            new_data = pd.DataFrame([[case_no, case_year, p1_type, p1_name, p2_type, p2_name, circle, case_type, court, court_name, subject, h_date, decision, lawyer, status]], 
                                   columns=st.session_state.cases.columns)
...
with tab2:
    ...
    if not st.session_state.cases.empty:
        ...
        urgent = df[df['تاريخ الجلسة'] <= next_week]
...
with tab4:
    ...
    if not st.session_state.cases.empty:
        archived = st.session_state.cases[st.session_state.cases["الحالة"] == "منتهية"]
...
with tab5:
    ...
    if search and not st.session_state.cases.empty:
