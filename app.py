...
if st.session_state.page == "alerts":
    ...
    total = len(filtered[filtered["status"] == "متداولة"])
    judgments = len(filtered[df["judgment_result"].isin(["لصالح الهيئة", "ضد الهيئة"])])
    ...
    col1.metric("إجمالي القضايا المتداولة", total)
    col2.metric("إجمالي الأحكام", judgments)
    col3.metric("لصالح الهيئة", win)
    col4.metric("ضد الهيئة", lose)

    st.markdown("---")

    st.markdown("## 📋 بيان الدعاوى المتداولة")

    for i, r in filtered[filtered["status"] == "متداولة"].iterrows():
        st.markdown(f"""
        ---
