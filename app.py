...
# ==========================================================
    ...
    st.markdown("---")
    # ==========================================================
# التقارير - النسخة الكاملة حسب طلبك
# ==========================================================
    ...
    subject_filter = ""
    if "حسب موضوع" in report_type:
        subject_filter = st.text_input("موضوع الدعوى (للتصفية)")
    ...
    if st.button("📄 عرض التقرير", use_container_width=True, type="primary"):
        if report_type == "الإحصائيات":
            ...
            cur.execute("""SELECT COUNT(DISTINCT case_id) FROM sessions 
                        WHERE procedure LIKE '%حكم%'""")
            total_judgments = cur.fetchone()[0] or 0

            cur.execute("""SELECT COUNT(DISTINCT case_id) FROM sessions 
                        WHERE procedure LIKE '%حكم%' AND procedure LIKE '%صالح%'""")
            favorable = cur.fetchone()[0] or 0

            cur.execute("""SELECT COUNT(DISTINCT case_id) FROM sessions 
                        WHERE procedure LIKE '%حكم%' AND procedure LIKE '%ضد%'""")
            against = cur.fetchone()[0] or 0

            st.success(f"**إحصائيات الفترة**")
            c1,c2,c3,c4 = st.columns(4)
            with c1: st.metric("إجمالي القضايا المتداولة", total_cases)
            with c2: st.metric("إجمالي الأحكام", total_judgments)
            with c3: st.metric("الأحكام للصالح", favorable, delta_color="normal")
            with c4: st.metric("الأحكام للضد", against, delta_color="inverse")

        else:
            # باقي التقارير (جداول)
            query = """
            SELECT 
                c.id, c.case_number, c.judicial_year, c.circuit, c.case_category,
                c.court_name, c.mission, c.plaintiff, c.defendant, c.subject,
                s.session_date, s.procedure, c.notes
            FROM cases c
            ...
            if "متداولة" in report_type and "حسب موضوع" not in report_type:
                query += " AND (s.procedure NOT LIKE '%حكم%' OR s.procedure IS NULL)"
            elif "أحكام" in report_type:
                query += " AND s.procedure LIKE '%حكم%'"
            ...
            if results:
                ...
                for i, row in enumerate(results, 1):
                    judgment_result = "للصالح" if "صالح" in str(row[11]) else "للضد" if "ضد" in str(row[11]) else ""
                    data.append({
                        "م": i,
                        "رقم القضية": f"{row[1]} لسنة {row[2]}",
                        "الدائرة والنوع": f"{row[3]} - {row[4]}",
                        "المحكمة": row[5],
                        "المأمورية": row[6] or "-",
                        "الخصوم": f"{row[7]} ضد {row[8]}",
                        "موضوع الدعوى": row[9],
                        "آخر جلسة": row[10],
                        "الإجراء": row[11],
                ...
                with col_pdf:
                    if st.button("📄 تصدير PDF", use_container_width=True):
                        st.info("سيتم إضافة تصدير PDF قريباً")
                with col_word:
                    if st.button("📝 تصدير Word", use_container_width=True):
                        st.info("سيتم إضافة تصدير Word قريباً")
            else:
                st.warning("لا توجد نتائج في هذه الفترة")
