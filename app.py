...
# ==========================================================
# التقارير - الكود الكامل
# ==========================================================

elif st.session_state.page == "reports":

    st.markdown("""
    <div style="background:#4E342E; border:2px solid gold; border-radius:18px; padding:20px; text-align:center; color:white; font-size:32px; font-weight:bold;">
    📊 التقارير
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("📄 عرض التقرير", use_container_width=True, type="primary"):
        ...
        st.dataframe({
            ...
            "تاريخ الحكم": ["2025-06-20", "2025-06-25"],
            "منطوق الحكم": ["للصالح", "للضد"],
            "ملاحظات": ["-", "-"]
        }, use_container_width=True)
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.button("📄 فتح PDF", use_container_width=True)
            st.button("📥 تحميل PDF", use_container_width=True)
        with c2:
            st.button("📝 فتح Word", use_container_width=True)
            st.button("📥 تحميل Word", use_container_width=True)
