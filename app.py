...
if st.session_state.page == "reports":
    ...
    data_cases = filtered[filtered["status"] == "متداولة"]

    if len(data_cases) == 0:
        st.warning("لا توجد دعاوى خلال الفترة المحددة")
    else:
        for _, r in data_cases.iterrows():
            st.markdown(f"""
---
**رقم القضية:** {r["case_no"]}/{r["judicial_year"]}  
**الخصوم:** {r["claimant"]} ضد {r["defendant"]}  
**المحكمة:** {r["court_name"]}  
**الموضوع:** {r["subject"]}  
**آخر جلسة:** {r["session_date"].date() if not pd.isna(r["session_date"]) else "—"}
""")
