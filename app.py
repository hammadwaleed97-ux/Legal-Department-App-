...
def create_word(rows, user_name):
    ...
    for row in rows:
        doc.add_paragraph(f"رقم القضية: {row[6]} / {row[7]}")
        doc.add_paragraph(f"الخصوم: {row[3]} ضد {row[5]}")
        doc.add_paragraph(f"الموضوع: {row[13]}")
        doc.add_paragraph(f"الحالة: {row[17]}")
        doc.add_paragraph("----------------------")
...
def create_pdf(rows, user_name):
    ...
    for row in rows:
        p.drawString(50, y, f"{row[6]}/{row[7]} - {row[3]} ضد {row[5]}")
        y -= 20
