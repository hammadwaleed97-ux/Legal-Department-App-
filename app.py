...
""")
    ...
    report_type = st.selectbox(
        ...
        [
            "بيان بالدعاوى المتداولة",
            "بيان بالأحكام للصالح والضد",
            "بيان بالأحكام للصالح",
            "بيان بالأحكام للضد",
            "بيان بالدعاوى المتداولة حسب موضوع الدعوى",
            "بيان بالأحكام حسب موضوع الدعوى",
            "الإحصائيات"
    ...
    # فلتر موضوع الدعوى
    subject_filter = st.text_input("موضوع الدعوى") if "حسب موضوع" in report_type else ""
    ...
    if st.button("📄 عرض التقرير", use_container_width=True, type="primary"):
        ...
        st.markdown(f"""
        <div style="text-align:center; margin:25px 0; line-height:2.2;">
            <h2 style="color:#FFD700;">الهيئة القومية للتأمين الاجتماعى</h2>
            <h3>الإدارة المركزية للشئون القانونية</h3>
            <h3>الإدارة العامة للقضايا</h3>
            <h3>ديوان عام منطقة البحيرة</h3>
            <br>
            <b>{report_type}</b><br>
            خلال الفترة من {from_date} حتى {to_date}<br>
            طرف الاستاذ/ {lawyer if lawyer else ".................."}
        </div>
        """, unsafe_allow_html=True)

        # عرض الجدول
        st.success(f"✅ تم عرض {report_type}")
        st.dataframe({
            "م": [1, 2],
            "رقم القضية": ["123/2025", "456/2025"],
            "الدائرة": ["دائرة 1", "دائرة 2"],
            "اسم المحكمة": ["الابتدائية", "الاستئناف"],
            "الخصوم": ["الهيئة ضد فلان", "فلان ضد الهيئة"],
            "موضوع الدعوى": ["تعويض تأميني", "مستحقات"],
            "آخر جلسة / تاريخ الحكم": ["2025-06-20", "2025-06-25"],
            "الإجراء / منطوق الحكم": ["للصالح", "للضد"],
            "ملاحظات": ["-", "-"]
        }, use_container_width=True)

        # التذييل
        st.markdown(f"""
        <div style="text-align:center; margin:40px 0; color:white; font-size:18px;">
            تفضلوا بقبول وافر الاحترام<br><br>
            عضو الإدارة      مدير الإدارة<br><br>
            تحرر في: {datetime.now().strftime("%Y-%m-%d")}
        </div>
        """, unsafe_allow_html=True)

        # أزرار التصدير
        st.markdown("### أزرار التصدير")
        c1, c2 = st.columns(2)
        with c1:
            if st.button("📄 فتح PDF", use_container_width=True):
                st.info("جاري فتح ملف PDF...")
            if st.button("📥 تحميل PDF", use_container_width=True):
                st.success("تم تحميل PDF")
        with c2:
            if st.button("📝 فتح Word", use_container_width=True):
                st.info("جاري فتح ملف Word...")
            if st.button("📥 تحميل Word", use_container_width=True):
                st.success("تم تحميل Word")
