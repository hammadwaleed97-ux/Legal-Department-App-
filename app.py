...
if st.session_state.page == "archive":
    ...
    if rows:
        ...
        for row in rows:
            ...
            st.write(
                f"{row[7]}/{row[6]} | {row[8]} | {row[9]} | "
                f"{row[11]} | {row[3]} ضد {row[5]} | "
                f"{row[13]} | الحكم: {row[20]}"
            )
...
if st.session_state.page == "all_cases":
    ...
    if not rows:
        ...
        selected_case = st.selectbox(
            "اختر القضية لفتحها",
            rows,
            format_func=lambda x: f"{x[6]} / {x[7]}"
        )

        if st.button("📂 فتح القضية"):

            st.session_state.selected_case = selected_case[0]
