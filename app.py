...
""")
    ...
    <h3>الإدارة المركزية للإدارات القانونية - الإدارة العامة للقضايا</h3>
    <h3>ديوان عام منطقة البحيرة</h3>
    </div>
    """, unsafe_allow_html=True)

    # خانات الفلترة المطلوبة
    st.markdown("---")
    col_f1, col_f2, col_f3 = st.columns(3)
    
    with col_f1:
        # الفلتر الأساسي للصالح والضد
        judgment_filter = st.selectbox("تصنيف الأحكام", ["جميع الأحكام", "أحكام للصالح", "أحكام للضد"])
    with col_f2:
        subject_filter = st.text_input("موضوع الدعوى (اختياري)")
    with col_f3:
        lawyer_name = st.text_input("طرف الأستاذ المحامي")

    # تحديد التاريخ
    col_d1, col_d2 = st.columns(2)
    f_date = col_d1.date_input("من تاريخ")
    t_date = col_d2.date_input("إلى تاريخ")

    if st.button("استخراج التقرير"):
        st.markdown(f"""
        <div style="text-align:center; color:white; margin:15px 0;">
        <h3 style="color:#FFD700;">بيان {judgment_filter} {('عن ' + subject_filter) if subject_filter else ''}</h3>
        <p>خلال الفترة من {f_date} حتى {t_date} | طرف الأستاذ/ {lawyer_name}</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        <tr style="background:#5A3A22; color:#FFD700;">
            <th style="padding:10px; border:1px solid gold;">م</th>
            <th style="padding:10px; border:1px solid gold;">رقم القضية</th>
            <th style="padding:10px; border:1px solid gold;">السنة القضائية</th>
            <th style="padding:10px; border:1px solid gold;">الدائرة والنوع</th>
            <th style="padding:10px; border:1px solid gold;">اسم المحكمة</th>
            <th style="padding:10px; border:1px solid gold;">المأمورية</th>
            <th style="padding:10px; border:1px solid gold;">الخصوم</th>
            <th style="padding:10px; border:1px solid gold;">موضوع الدعوى</th>
            <th style="padding:10px; border:1px solid gold;">تاريخ جلسة الحكم</th>
            <th style="padding:10px; border:1px solid gold;">منطوق الحكم</th>
            <th style="padding:10px; border:1px solid gold;">الصالح/الضد</th>
            <th style="padding:10px; border:1px solid gold;">ملاحظات</th>
        </tr>
        <!-- هنا يتم عرض البيانات من قاعدة البيانات -->
        </table>
        """, unsafe_allow_html=True)

        # التذييل المهني الموحد
        st.markdown(f"""
        <div style="margin-top:30px; color:white; font-weight:bold;">
        <p>وتفضلوا بقبول وافر الاحترام</p>
        <div style="display:flex; justify-content:space-between;">
        <span>عضو الإدارة: .................</span>
        <span>مدير الإدارة: .................</span>
        </div>
        <p>تحرر في: {datetime.now().strftime('%Y-%m-%d')}</p>
        </div>
        """, unsafe_allow_html=True)

        # أزرار التصدير (PDF/Word)
        c1, c2, c3, c4 = st.columns(4)
        c1.button("فتح PDF"); c2.button("فتح Word"); c3.button("تحميل PDF"); c4.button("تحميل Word")
