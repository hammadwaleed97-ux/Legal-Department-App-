...
with tab3:
    ...
    # التقرير المطلوب: جدول منظم للطباعة
    st.dataframe(st.session_state.cases[['رقم الدعوى', 'الدائرة', 'نوع الدعوى', 'المحكمة', 'اسم الطرف الأول', 'اسم الطرف الثاني', 'تاريخ الجلسة', 'القرار']], use_container_width=True)
...
with tab5:
    st.subheader("البحث")
    search = st.text_input("أدخل كلمة البحث (رقم، اسم، دائرة)")
    if search:
