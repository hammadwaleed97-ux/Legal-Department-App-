...
# =====================================
# الحصر العام للقضايا - نسخة مطورة
# =====================================
...
if st.session_state.page == "all_cases":
    ...
    with colf1:
        search = st.text_input("🔍 بحث (رقم القضية / الخصوم)", "")

    with colf2:
        court_filter = st.text_input("🏛️ المحكمة", "")

    with colf3:
        year_filter = st.text_input("📅 السنة", "")
    ...
    if search:
        query += " AND (case_no LIKE ? OR plaintiff LIKE ? OR defendant LIKE ?)"
        params.extend([f"%{search}%", f"%{search}%", f"%{search}%"])

    if court_filter:
        query += " AND court_name LIKE ?"
        params.append(f"%{court_filter}%")

    if year_filter:
        query += " AND judicial_year LIKE ?"
    ...
    with colp1:
        if st.button("⬅️ السابق") and st.session_state.page_num > 1:
            st.session_state.page_num -= 1

    with colp2:
        st.write(f"📄 صفحة {st.session_state.page_num} من {total_pages}")

    with colp3:
        if st.button("➡️ التالي") and st.session_state.page_num < total_pages:
            st.session_state.page_num += 1
    ...
    if not page_rows:
        ...
        for row in page_rows:
            ...
            if last_update:
                last_session = last_update["next_session_date"] or "-"
                last_reason = last_update["status_reason"] or "-"
            ...
            with st.container():
                ...
                with col1:
                    st.markdown(
                        f"""
                        **{safe(row['case_no'])}**  
                        {safe(row['plaintiff'])} ضد {safe(row['defendant'])}  
                        🏛️ {safe(row['court_name'])} | 📅 {safe(row['judicial_year'])} | ⚖️ {safe(row['case_type'])}  
                        📌 الموضوع: {safe(row['subject'])}  
