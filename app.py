...
# Build the comprehensive app code
comprehensive_code = r'''import streamlit as st
import json
import os
    ...
    with t1:
        with st.form("memo_f"):
            ...
            if sw and "memo" in st.session_state:
                ...
                st.markdown(f'<a href="data:text/html;base64,{b64}" download="مذكرة_{m_num}.html">⬇️ تحميل HTML</a>', unsafe_allow_html=True)
    ...
    with t1:
        with st.form("fatwa_f"):
            ...
            if st.form_submit_button("💾 حفظ", use_container_width=True):
                ...
                d["activities"].append({"action": f"فتوى {ft}", "date": datetime.now().strftime("%Y-%m-%d %H:%M")})
                save_data(d); st.success("✅ تم الحفظ!")
        ...
        if d["fatwas"]:
            for f in d["fatwas"][::-1]:
                with st.expander(f"{f.get('type')} - {f.get('requester')}"):
                    st.write(f"**السؤال:** {f.get('question')}"); st.write(f"**الرأي:** {f.get('opinion')}"); st.write(f"**النتيجة:** {f.get('result')}")
    ...
    with t1:
        with st.form("inv_f"):
            ...
            if sub and num and violator:
                ...
                d["activities"].append({"action": f"تحقيق {num}", "date": datetime.now().strftime("%Y-%m-%d %H:%M")})
                save_data(d); st.success("✅ تم الحفظ!")
    ...
    with t2:
        ...
        if d["investigations"]:
            for i in d["investigations"][::-1]:
                with st.expander(f"تحقيق {i.get('number')} - {i.get('violator')}"):
                    st.write(f"**المخالف:** {i.get('violator')}"); st.write(f"**نوع المخالفة:** {i.get('violationType')}"); st.write(f"**الحالة:** {i.get('status')}")
    ...
    with t1:
        with st.form("lib_f"):
            ...
            if st.form_submit_button("⬆️ رفع", use_container_width=True):
                ...
                d["activities"].append({"action": f"رفع وثيقة: {title}", "date": datetime.now().strftime("%Y-%m-%d %H:%M")})
                save_data(d); st.success("✅ تم الرفع!")
    ...
    with at[2]:
        if d["fatwas"]:
            for i, f in enumerate(d["fatwas"]):
                with st.expander(f"#{i+1} - {f.get('type')} - {f.get('requester')}"):
                    st.write(f"**الموضوع:** {f.get('question')}"); st.write(f"**النتيجة:** {f.get('result')}")
        else: st.info("لا توجد فتاوى")
...
# ================== البحث المتقدم ==================
    ...
    if "res" in st.session_state and st.session_state.res:
        st.success(f"تم العثور على {len(st.session_state.res)} نتيجة")
...
# Save
with open('/mnt/agents/output/app_comprehensive.py', 'w', encoding='utf-8') as f:
    f.write(comprehensive_code)

size = os.path.getsize('/mnt/agents/output/app_comprehensive.py')
lines = len(comprehensive_code.split('\n'))
print(f"✅ Comprehensive app saved!")
